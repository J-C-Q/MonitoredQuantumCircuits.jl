...
function MonitoredQuantumCircuits.apply!(
    ...
    if keep_result
        ...
        projectXX!(backend.state, p1,p2; keep_result, phases)
        res = false
    end
    return res
end
...
function MonitoredQuantumCircuits.apply!(
    ...
    if keep_result
        _, res = projectXXrand!(backend.state, b.qubit1, b.qubit2, backend.operator)
        res = Bool(res/2)
        push!(backend.measurements, res)
        push!(backend.measured_qubits, ancilla)
    else
        projectXX!(backend.state, b.qubit1, b.qubit2, backend.operator; keep_result, phases)
        res = false
    end
    return res
end
