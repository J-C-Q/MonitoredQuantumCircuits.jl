...
function simulate(path::String; L=12, averaging=10, depth=1000, resolution=10)
    ...
    subsystems = MonitoredQuantumCircuits.subsystems(geometry, 2; cutType=:Z_VERTICAL)
    ...
    Threads.@threads for (px, py, pz) in points
        ...
        for _ in 1:averaging
            ...
            if QuantumClifford.state_entropy(result.stab) < 1e-10
                entropie += QuantumClifford.entanglement_entropy(result.stab, subsystems)
            end
            ProgressMeter.next!(progressMeter)
        end
        entropie /= averaging
        JLD2.save(
            "$path/Entropy_L=$(L)_px=$(px)_py=$(py)_pz=$(pz)_averaging=$(averaging).jld2",
            "entropie", entropie,
            "probs", (px, py, pz),
            "depth", depth)
    end
